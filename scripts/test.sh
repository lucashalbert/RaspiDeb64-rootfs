...
ROOTFS_TAR="${ROOTFS_TAR:-${BUILDS}/rootfs-${BUILD_ARCH}-${VARIANT}-${OS_RELEASE}-${OS_VERSION}-${BUILD_VERSION}.tar.gz}"
...
cd "${ROOTFS_DIR}" && rspec --format documentation --color ${WORKDIR}/test
